...
export default function Sidebar({ role, userName }: SidebarProps) {
  ...
  return (
    <aside
      className={`flex flex-col border-r border-white/10 bg-surface transition-all duration-300 ${
        collapsed ? "w-16" : "w-64"
      }`}
    >
      {/* Header */}
      <div className="flex h-16 items-center justify-between px-4 border-b border-white/10">
        {!collapsed && (
          <span className="text-lg font-bold text-accent truncate">
            🏠 CRM
          </span>
        )}
        <button
          onClick={() => setCollapsed(!collapsed)}
          className="rounded-lg p-1.5 text-muted hover:bg-white/5 transition-colors"
